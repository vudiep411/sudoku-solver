...
class Annealer(object):
    ...
    def anneal(self, screen=None, is_display=False):
        ...
        while step < self.steps and not self.user_exit:
            ...
            if self.updates > 1:
                ...
                if screen and is_display:
                    screen.fill((255, 255, 255))
                    draw_grid(screen=screen)        
                    draw_sudoku(self.state, screen)
                    font = pygame.font.Font(None, 36)
                    text = font.render(format_time(remaining_time), True, BLACK)
                    text2 = font.render(str(E), True, BLACK) 
                    screen.blit(text, (200,525))
                    screen.blit(text2, (120,525))
                    pygame.display.update()
                    remaining_time = 210 - (time.time() - start_time)
                if remaining_time < 0:
                    break
